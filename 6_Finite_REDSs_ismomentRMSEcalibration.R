...
simulatedbatch_bias_Monte<-foreach(batchnumber =c((1:length(allkurtlognorm))), .combine = 'rbind') %dopar% {
  ...
  write.csv(RMSEbataches2,paste("finite_lognorm_Ismomentscalibration_raw",samplesize,round(kurtx,digits = 1),".csv", sep = ","), row.names = FALSE)
...
write.csv(Optimum_RMSE,paste("finite_Ismoments_lognorm.csv", sep = ","), row.names = FALSE)
...
simulatedbatch_bias_Monte_SE<-foreach(batchnumber =c((1:length(allkurtlognorm))), .combine = 'rbind') %dopar% {
  ...
  SEbataches<- read.csv(paste("finite_lognorm_Ismomentscalibration_raw",samplesize,round(kurtx,digits = 1),".csv", sep = ","))
...
write.csv(simulatedbatch_bias_Monte_SE,paste("finite_lognorm_Ismomentscalibration_raw_error",samplesize,".csv", sep = ","), row.names = FALSE)
...
simulatedbatch_bias_Monte<-foreach(batchnumber =c((1:length(allkurtgnorm))), .combine = 'rbind') %dopar% {
  ...
  RMSEbataches<- read.csv(paste("finite_gnorm_Icalibration_raw",samplesize,round(kurtx,digits = 1),".csv", sep = ","))
  ...
  write.csv(RMSEbataches2,paste("finite_gnorm_Ismomentscalibration_raw",samplesize,round(kurtx,digits = 1),".csv", sep = ","), row.names = FALSE)
...
write.csv(simulatedbatch_bias_Monte,paste("finite_gnorm_Ismomentscalibration_raw",samplesize,".csv", sep = ","), row.names = FALSE)

Optimum_RMSE<-simulatedbatch_bias_Monte[,1:8]

write.csv(Optimum_RMSE,paste("finite_Ismoments_gnorm.csv", sep = ","), row.names = FALSE)
...
simulatedbatch_bias_Monte_SE<-foreach(batchnumber =c((1:length(allkurtgnorm))), .combine = 'rbind') %dopar% {
  ...
  SEbataches<- read.csv(paste("finite_gnorm_Ismomentscalibration_raw",samplesize,round(kurtx,digits = 1),".csv", sep = ","))
...
write.csv(simulatedbatch_bias_Monte_SE,paste("finite_gnorm_Ismomentscalibration_raw_error",samplesize,".csv", sep = ","), row.names = FALSE)


finite_I_gnorm<-( read.csv(("finite_Ismoments_gnorm.csv")))
finite_I_lognorm<-( read.csv(("finite_Ismoments_lognorm.csv")))
...
finite_I_gnorm<- read.csv(("finite_Ismoments_lognorm.csv"))
names(finite_I_lognorm)<-names(finite_I_gnorm)

asymptotic_I_lognorm<- read.csv(paste("asymptotic_Ismoments.csv", sep = ","))
...
write.csv(all1,paste("Ismoments_values.csv", sep = ","), row.names = FALSE)
